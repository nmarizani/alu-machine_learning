...
class DeepNeuralNetwork:
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        ...
        # Calculate the derivative of the cost w.r.t output layer activation
        dZ = cache[f"A{L}"] - Y
        ...
        for l in reversed(range(1, L + 1)):
            ...
            self.weights[f"W{l}"] = self.weights[f"W{l}"] - alpha * dW
            self.weights[f"b{l}"] = self.weights[f"b{l}"] - alpha * db
