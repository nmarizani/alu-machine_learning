...
def adjugate(matrix):
    ...
    :param matrix: List of lists representing a square matrix.
    :return: Adjugate matrix (list of lists).
    """
    if (not isinstance(matrix, list) or
            not all(isinstance(row, list) for row in matrix)):
        raise TypeError("matrix must be a list of lists")
    ...
    adjugate_matrix = [[cofactor_matrix[j][i] for j in range(rows)]
                       for i in range(rows)]
...
def inverse(matrix):
    ...
    :param matrix: List of lists representing a square matrix.
    :return: Inverse matrix (list of lists) or None if matrix is singular.
    """
    if (not isinstance(matrix, list) or
            not all(isinstance(row, list) for row in matrix)):
        raise TypeError("matrix must be a list of lists")
    ...
    if det == 0:
        return None  # Singular matrix (non-invertible)

    adj_matrix = adjugate(matrix)

    # Compute inverse by dividing each element of adjugate by determinant
    inverse_matrix = [[adj_matrix[i][j] / det for j in range(rows)]
                      for i in range(rows)]
